...
import os
from ocr.domain.entities import Section, TextParagraph, Table  # noqa: F401
from ocr.domain.repositories import IPDFGeneratorRepository  # noqa: F401
...
class FPDF2GeneratePDFRepository(IPDFGeneratorRepository):
    ...
    def _insert_section(self, pdf: FPDF, section: Section):
        if section.paragraphs:
            self.logger.info(f"Inserting {len(section.paragraphs)} paragraphs in section.")
    ...
    def _insert_paragraph(self, pdf: FPDF, paragraph: TextParagraph):
        ...
        self.logger.debug(f"Moving to page {paragraph.page_number} for paragraph insertion.")
